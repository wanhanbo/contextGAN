...
for epoch in range(opt.n_epochs):
    for i, (imgs, masked_imgs, masked_size) in enumerate(dataloader):
        ...
        # g_pixel += pixelwise_loss(gen_imgs_unmasked_parts[:, :, start_pixel:, :start_pixel], unmasked_parts[:, :, start_pixel:, :start_pixel]) #left_condition


        # Total loss
        g_loss = 0.4 * g_adv + 0.6 * g_pixel

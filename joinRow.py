...
path="D:/codes/gan/contextGan_data/contextGAN_bs128_all__82_top_512/93799.pth"


model = Generator(channels=opt.channels)
...
#         mem[i][j] = model(masked_img,noise)
...
for i in range(h):
    noise= Variable(Tensor(np.random.normal(0, 1, (1, opt.latent_dim))))
    if(i > 0):
        masked_img=generete_top_condition(mem[0][0])
        mem[i][j] = model(masked_img,noise)
        save_image(masked_img[0], "%s/%s_masked.png" % (opt.out_dir, i*w+j), normalize=True)
        save_image(mem[i][j][0], "%s/%s_gen.png" % (opt.out_dir, i*w+j), normalize=True)
